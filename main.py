...
async def download_video(request: DownloadRequest):
    format_str = 'best' if request.quality == 'best' else f'bestvideo[height<={request.quality}]+bestaudio/best[height<={request.quality}]/best'
    try:
        with yt_dlp.YoutubeDL({'format': format_str, 'noplaylist': True}) as ydl:
            info = ydl.extract_info(request.url, download=False)
            return {"success": True, "download_url": info.get('url')}
